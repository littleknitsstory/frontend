...
let initialState: ICart = localStorage.getItem("cart")
...
const updateTotalPrice = (cartItem: ICartProduct[]) => {
  return cartItem.reduce(
    (acc, current) => acc + current.amount * convertToNumber(current.price, i18n.language),
    0,
  );
};
...
const cartSlice = createSlice({
  ...
  reducers: {
    ...
    updateProductPrice(state, action: PayloadAction<IProductDetails>) {
      state.products = state.products.map((product) =>
        product.id === action.payload.id ? { ...product, price: action.payload.price } : product,
      );
      state.totalPrice = updateTotalPrice(state.products);
      localStorage.setItem("cart", JSON.stringify(state));
    },
